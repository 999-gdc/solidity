...
ASTPointer<ASTNode> ASTJsonImporter::convertJsonToASTNode(Json::Value const& _json)
{
	...
	if (nodeType == "FormalDocumentation")
...
ASTPointer<FormalDocumentation> ASTJsonImporter::createDocumentation(Json::Value const&  _node)
{
	static string const textString = "text";
	static string const formatString = "format";

	astAssert(member(_node, textString).isString(), "'text' must be a string");
	astAssert(member(_node, formatString).isString(), "'format' must be a string");
	astAssert(member(_node, formatString) == "NatSpec", "'format' must specify 'NatSpec'");

	ASTPointer<ASTString> text = make_shared<ASTString>(_node[textString].asString());

	return createASTNode<FormalDocumentation>(
		_node,
		text
	);
}
